...
from crested.pl.patterns._modisco_results import _trim_pattern_by_ic, _get_ic
    ...
    for class_name in class_names:
        try:
            ...
            if not os.path.exists(output_file):
                ...
                modisco.io.save_hdf5(output_file, pos_patterns, neg_patterns, window_size=window)
...
def add_pattern_to_dict(
    ...
    Parameters:
    - p (dict): Pattern to add.
    - idx (int): Index for the new pattern.
    - cell_type (str): Cell type of the pattern.
    - pos_pattern (bool): Indicates if the pattern is a positive pattern.
    - all_patterns (dict): Dictionary containing all patterns.

    Returns:
    - dict: Updated dictionary with the new pattern.
    """
...
def match_to_patterns(
    ...
    pattern_id: str, 
    pos_pattern: bool, 
    all_patterns: Dict[str, Dict[str, Union[str, List[float]]]], 
    ...
    Parameters:
    - p (dict): Pattern to match.
    - idx (int): Index of the pattern.
    - cell_type (str): Cell type of the pattern.
    - pattern_id (str): ID of the pattern.
    - pos_pattern (bool): Indicates if the pattern is a positive pattern.
    - all_patterns (dict): Dictionary containing all patterns.
    - sim_threshold (float): Similarity threshold for matching patterns.
    - ic_threshold (float): Information content threshold for matching patterns.
    - verbose (bool): Flag to enable verbose output.

    Returns:
    - dict: Updated dictionary with matched patterns.
    """
    p['id'] = pattern_id
    if not all_patterns:
    ...
    if verbose:
        print(f'Match between {pattern_id} and {all_patterns[str(match_idx)]["pattern"]["id"]}')
...
def post_hoc_merging(
    ...
    Parameters:
    - all_patterns (dict): Dictionary of all patterns with metadata.
    - sim_threshold (float): Similarity threshold for merging patterns.
    - ic_discard_threshold (float): IC threshold below which patterns are discarded.
    - verbose (bool): Flag to enable verbose output of merged patterns.

    Returns:
    - dict: Updated patterns after merging and filtering with sequential keys.
    """
        ...
        for i, (idx1, pattern1) in enumerate(pattern_list):
            ...
            for j, (idx2, pattern2) in enumerate(pattern_list):
                ...
                if sim > sim_threshold:
                    ...
                    any_merged = True
                    if verbose:
                        print(f'Merging patterns: {pattern1["pattern"]["id"]} and {pattern2["pattern"]["id"]} with similarity {sim}')
    ...
    if verbose:
        ...
        print(f'Discarded {discarded_count} patterns below IC threshold {ic_discard_threshold} and with a single class instance:')
        print(discarded_ids)
    
    # Reindex the filtered patterns
    final_patterns = {str(new_idx): v for new_idx, (k, v) in enumerate(filtered_patterns.items())}
...
def merge_patterns(
    ...
    Parameters:
    - pattern1 (dict): First pattern with metadata.
    - pattern2 (dict): Second pattern with metadata.

    Returns:
    - dict: Merged pattern with updated metadata.
    """
...
def pattern_similarity(
    ...
    Parameters:
    - all_patterns (dict): Dictionary containing all patterns.
    - idx1 (int): Index of the first pattern.
    - idx2 (int): Index of the second pattern.
    ...
    """
    sim = max(
        match_score_patterns(all_patterns[str(idx1)]['pattern'], all_patterns[str(idx2)]['pattern']),
        match_score_patterns(all_patterns[str(idx2)]['pattern'], all_patterns[str(idx1)]['pattern'])
    )
    if plot:
...
def normalize_rows(arr: np.ndarray) -> np.ndarray:
    """
    Normalize the rows of an array such that the positive values are scaled by their maximum
    and negative values by their minimum absolute value.

    Parameters:
    - arr (np.ndarray): Input array to be normalized.

    Returns:
    - np.ndarray: The row-normalized array.
    """
...
def find_pattern(id_: str, pattern_dict: Dict) -> Union[int, None]:
    """
    Finds the index of a pattern by its ID.

    Parameters:
    - id_ (str): The ID of the pattern to find.
    - pattern_dict (dict): A dictionary containing pattern data.

    Returns:
    - int or None: The index of the pattern if found, otherwise None.
    """
...
def match_h5_files_to_classes(contribution_dir: str, classes: List[str]) -> Dict[str, Optional[str]]:
    """
    ...
    - classes (list): List of class names to match against file names.

    Returns:
    - dict: A dictionary where keys are class names and values are paths to the corresponding .h5 files if matched, None otherwise.
    """
...
def process_patterns(
    matched_files: Dict[str, Optional[str]], 
    sim_threshold: float = 0.5, 
    ...
    Parameters:
    - matched_files (dict): Dictionary with class names as keys and paths to HDF5 files as values.
    - sim_threshold (float): Similarity threshold for matching patterns.
    - trim_ic_threshold (float): Information content threshold for trimming patterns.
    - discard_ic_threshold (float): Information content threshold for discarding patterns.
    - verbose (bool): Flag to enable verbose output.

    Returns:
    - dict: All processed patterns with metadata.
    """
    all_patterns = {}
    counter = 0
    
    for ct_idx, cell_type in enumerate(matched_files.keys()):
        if verbose:
        ...
        for metacluster_name in metacluster_names:
            for p in hdf5_results[metacluster_name]:
                pattern_ids.append(f"{cell_type.replace(' ', '_')}_{metacluster_name}_{p}")
                patterns.append(hdf5_results[metacluster_name][p])
                pos_pat = metacluster_name == 'pos_patterns'
                pos_patterns.append(pos_pat)

        trimmed_patterns = [
            _trim_pattern_by_ic(pattern, pos_pattern, trim_ic_threshold) for pattern, pos_pattern in zip(patterns, pos_patterns)
        ]

        for idx, p in enumerate(trimmed_patterns):
            all_patterns = match_to_patterns(p, idx, cell_type, pattern_ids[idx], pos_patterns[idx], all_patterns, sim_threshold, discard_ic_threshold, verbose)

    all_patterns = post_hoc_merging(all_patterns=all_patterns, sim_threshold=sim_threshold, ic_discard_threshold=discard_ic_threshold, verbose=verbose)
...
def create_pattern_matrix(
    ...
    - normalize (bool): Flag to indicate whether to normalize the rows of the matrix.

    Returns:
    - np.ndarray: The resulting pattern matrix, optionally normalized.
    """
    ...
    for p_idx in all_patterns:
        ...
        for ct in p_classes:
            ...
            pattern_matrix[idx, int(p_idx)] = np.mean(all_patterns[p_idx]['classes'][ct]['contrib_scores'])
...
def generate_nucleotide_sequences(all_patterns: Dict) -> List[Tuple[str, np.ndarray]]:
    ...
    Returns:
    - list: List of tuples containing sequences and their normalized heights.
    """
...
def generate_image_paths(
    ...
    Parameters:
    - pattern_matrix (ndarray): Filtered 2D array of pattern data.
    - all_patterns (dict): Dictionary containing pattern data.
    - classes (list): List of class labels.
    - contribution_dir (str): Directory containing contribution scores and images.

    Returns:
    - list: List of image paths corresponding to the patterns.
    """
    image_paths = []
    ...
    for i in range(pattern_matrix.shape[1]):
        ...
        pattern_class = '_'.join(pattern_class_parts) if len(pattern_class_parts) > 1 else pattern_class_parts[0]
...
def find_pattern_matches(
    ...
    - q_val_thr (float): The threshold for q-value filtering. Default is 0.05.

    Returns:
    - Dict[int, Dict[str, List[str]]]: A dictionary with pattern indices as keys and a dictionary of matches as values.
    """
    ...
    for i, p_idx in enumerate(all_patterns):
        ...
        pattern_id = pattern_id_parts[-4] + '_' + pattern_id_parts[-3] + '.' + pattern_id_parts[-2] + '_' + pattern_id_parts[-1]
        ...
        if not matching_row.empty:
            ...
            for j in range(3):
                ...
                if qval_column in matching_row.columns and match_column in matching_row.columns:
...
def read_motif_to_tf_file(file_path: str) -> pd.DataFrame:
    ...
    Returns:
    - pd.DataFrame: A DataFrame containing the motif to TF mappings.
    """
...
def create_pattern_tf_dict(
    ...
    - motif_to_tf_df (pd.DataFrame): A DataFrame containing motif to TF mappings.
    - all_patterns (List[Dict[str, Any]]): A list of patterns with metadata.
    - cols (List[str]): A list of column names to extract TF annotations from.

    Returns:
    - Tuple[Dict, np.ndarray]: A tuple containing the pattern to TF mappings dictionary and an array of all unique TFs.
    """
    pattern_tf_dict: Dict[int, Dict[str, Any]] = {}
    all_tfs: List[str] = []

    for i, p_idx in enumerate(pattern_match_dict):
...
def create_tf_ct_matrix(
    ...
    Parameters:
    - pattern_tf_dict (Dict[int, Dict[str, Any]]): A dictionary with pattern indices and their TFs.
    - all_patterns (List[Dict[str, Any]]): A list of patterns with metadata.
    - df (pd.DataFrame): A DataFrame containing gene expression data.
    - classes (List[str]): A list of cell type classes.
    - log_transform (bool): Whether to apply log transformation to the gene expression values. Default is True.
    - normalize (bool): Whether to normalize the contribution scores across the cell types. Default is True.

    Returns:
    - Tuple[np.ndarray, List[str]]: A tuple containing the TF-cell type matrix and the list of TF pattern annotations.
    """
    ...
    for p_idx in pattern_tf_dict:
        ...
        for ct in all_patterns[p_idx]['classes']:
            idx = np.argwhere(np.array(classes) == ct)[0][0]
            ct_contribs[idx] = np.mean(all_patterns[p_idx]['classes'][ct]['contrib_scores'])
