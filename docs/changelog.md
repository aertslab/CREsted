...
- Rotated logos (`rotate=True`, used e.g. for the vertical PWM/motif logos in {mod}`crested.pl.modisco`) now render as vector graphics via a data-space transform instead of being rasterized to an image and rotated with `np.rot90` — faster and no longer lossy/blurry.
