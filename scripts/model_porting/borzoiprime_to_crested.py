...
# Port weights from the official Baskerville-based Borzoi Prime model to the CREsted-based model.
...
saved_models_dir = "xxx"  # From links at https://github.com/calico/borzoi-paper/tree/main/extensions/prime, for f0-f3 and model0/model1
...
conv_lookup = {
    ...
    "separable_conv1d_2" : "upsampling_separable_3",
}
...
bnorm_lookup = {
    ...
    "sync_batch_normalization_7": "unet_skip_3_batchnorm",
    "sync_batch_normalization_8": "upsampling_conv_2_batchnorm",
    "sync_batch_normalization_9": "unet_skip_2_batchnorm",
    "sync_batch_normalization_10": "upsampling_conv_3_batchnorm",
    "sync_batch_normalization_11": "unet_skip_1_batchnorm",
}
...
pointwise_lookup = {
    ...
    'dense_17': 'unet_skip_3_conv',
    'dense_18': 'upsampling_conv_2_conv',
    'dense_19': 'unet_skip_2_conv',
    'dense_20': 'upsampling_conv_3_conv',
    'dense_21': 'unet_skip_1_conv',
    'dense_22': 'head_0'
}

mouse_lookup = {"dense_23": "head_1"}
...
def copy_convdense(mod, layers, name):
    ...
    if len(mod.weights) >= 2:
        assert conv[1].shape == mod.weights[1].shape, f"shape {conv[1].shape} != {mod.weights[1].shape} ({name})"
    if len(mod.weights) >= 3:
        assert conv[2].shape == mod.weights[2].shape, f"shape {conv[2].shape} != {mod.weights[2].shape} ({name})"
...
def copy_dense_to_pointwise(mod, layers, name):
    ...
    assert dense[0].shape == mod.weights[0].shape, f"shape {dense[0].shape} != {mod.weights[0].shape} ({name})"
    assert dense[1].shape == mod.weights[1].shape, f"shape {dense[1].shape} != {mod.weights[1].shape} ({name})"
...
def copy_bn(mod, layers, name):
    ...
    bn = [bn_w,
          ...
          bn_mov_mean.flatten(),
          bn_mov_var.flatten()]
    assert bn[0].shape == mod.weights[0].shape, f"shape {bn[0].shape} != {mod.weights[0].shape} ({name})"
    assert bn[1].shape == mod.weights[1].shape, f"shape {bn[1].shape} != {mod.weights[1].shape} ({name})"
    assert bn[2].shape == mod.weights[2].shape, f"shape {bn[2].shape} != {mod.weights[2].shape} ({name})"
    assert bn[3].shape == mod.weights[3].shape, f"shape {bn[3].shape} != {mod.weights[3].shape} ({name})"
    mod.set_weights(bn)
...
def copy_ln(mod, layers, name):
    ...
    ln = [ln_w, ln_b]
    assert ln[0].shape == mod.weights[0].shape, f"shape {ln[0].shape} != {mod.weights[0].shape} ({name})"
    assert ln[1].shape == mod.weights[1].shape, f"shape {ln[0].shape} != {mod.weights[0].shape} ({name})"
    mod.set_weights(ln)
...
def copy_mhsa(mod, layers, name):
    ...
    for i in range(len(mhsa)):
        assert mhsa[i].shape == mod.weights[i].shape, f"shape {mhsa[i].shape} != {mod.weights[i].shape} (index {i}) ({name})"
...
for i, (human_tf_path, mouse_tf_path) in enumerate(weights_paths):
    model = crested.tl.zoo.borzoi(
        seq_len=524288,
        num_classes=[5431, 1774],
        target_length=12288,
        unet_connections = [4, 5, 6],
    ...
    with h5py.File(human_tf_path) as human_weights, h5py.File(mouse_tf_path) as mouse_weights:
        for baskerville_name, crested_name in conv_lookup.items():
            copy_convdense(model.get_layer(crested_name), human_weights['model_weights'], baskerville_name)
        for baskerville_name, crested_name in bnorm_lookup.items():
            copy_bn(model.get_layer(crested_name), human_weights['model_weights'], baskerville_name)
        for baskerville_name, crested_name in lnorm_lookup.items():
            copy_ln(model.get_layer(crested_name), human_weights['model_weights'], baskerville_name)
        for baskerville_name, crested_name in mha_lookup.items():
            copy_mhsa(model.get_layer(crested_name), human_weights['model_weights'], baskerville_name)
        for baskerville_name, crested_name in pointwise_lookup.items():
            copy_dense_to_pointwise(model.get_layer(crested_name), human_weights['model_weights'], baskerville_name)
        for baskerville_name, crested_name in mouse_lookup.items():
            copy_dense_to_pointwise(model.get_layer(crested_name), mouse_weights['model_weights'], baskerville_name)

    # Check if all weights were used
    layers_ported = {l for lookup_dict in [conv_lookup, bnorm_lookup, lnorm_lookup, mha_lookup, pointwise_lookup, mouse_lookup] for l in lookup_dict.values()}
    ...
    model.save(os.path.join(output_dir, f"borzoiprime_crested_fold{i}.keras"))
    model.save_weights(os.path.join(output_dir, f"borzoiprime_crested_fold{i}.weights.h5"))
    print(f"Keras model saved to disk!\nModel: {os.path.join(output_dir, f'borzoiprime_crested_fold{i}.keras')}\nWeights: {os.path.join(output_dir, f'borzoiprime_crested_fold{i}.weights.h5')}")

    model_human = keras.Model(model.inputs, model.outputs[0])
    model_human.save(os.path.join(output_dir, f"borzoiprime_crested_human_fold{i}.keras"))
    model_mouse = keras.Model(model.inputs, model.outputs[1])
    model_mouse.save(os.path.join(output_dir, f"borzoiprime_crested_mouse_fold{i}.keras"))
